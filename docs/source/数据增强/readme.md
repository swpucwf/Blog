# 图像增强
## 目标检测
- [进行图像增广（数据扩充）的15种功能总结和Python代码实现](https://cloud.tencent.com/developer/article/1662864?areaId=106001)
- [目标检测】数据增强：YOLO官方数据增强实现/imgaug的简单使用](https://cloud.tencent.com/developer/article/2113438?areaId=106001)

## OCR
- [OCR -- 训练数据扩增的方法](https://cloud.tencent.com/developer/article/1435669?areaSource=102001.10&traceId=ZfdqeHZ0i6BvJsoYJRvMA)
- [最快最好用的数据增强库「albumentations」 一文看懂用法](https://blog.csdn.net/u014264373/article/details/114144303)
    - [官方文档](https://albumentations.ai/docs/getting_started/simultaneous_augmentation/)
    - [手写体数据增强](https://zhuanlan.zhihu.com/p/636127026)
    - [对应格式的数据增强](https://aistudio.baidu.com/projectdetail/4334420)
    - [使用教程](https://juejin.cn/post/7070668336010362910)
